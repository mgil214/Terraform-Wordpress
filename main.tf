...
resource "aws_default_route_table" "stg_private_route" {
  default_route_table_id = aws_vpc.stg_vpc.default_route_table_id
  tags = {
    Name = "stg_private"
  }
}
...
resource "aws_security_group" "stg_private_sg" {
  ...
  ingress {
    ...
    to_port     = 2049
    protocol    = "TCP"
    cidr_blocks = [var.stg_cidr, aws_instance.wordpress_stg.id]
  }
...
//resource "aws_efs_mount_target" "prod_efs_mount" {
